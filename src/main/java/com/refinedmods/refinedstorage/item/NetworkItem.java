...
public abstract class NetworkItem extends EnergyItem implements INetworkItemProvider {
    ...
    public ActionResult<ItemStack> onItemRightClick(World world, PlayerEntity player, Hand hand) {
        ...
        if (!world.isRemote) {
            applyNetwork(world.getServer(), stack, n -> n.getNetworkItemManager().open(player, player.getHeldItem(hand), player.inventory.currentItem), m -> player.sendMessage(m, Util.DUMMY_UUID));
        }
